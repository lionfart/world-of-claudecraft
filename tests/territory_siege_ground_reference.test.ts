...
} from '../src/render/territory_siege_ground_reference';
...
describe('territory siege character ground reference', () => {
  ...
  it('provides the raised siege surface to ground-following ability visuals', () => {
    const origin = territorySiegeOrigin(0);
    expect(territoryAwareGroundHeight(42, 4, origin.x, origin.z - 46)).toBe(
      DUNGEON_FLOOR_Y + TERRITORY_SIEGE_CITADEL_INNER_HEIGHT,
    );
  });
});
