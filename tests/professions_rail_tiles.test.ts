...
const gamepadActionDispatchSrc = read('../src/game/gamepad_action_dispatch.ts');
...
const entries = [
  ['index.html', read('../index.html').replace(/\s+/g, ' ').replace(/\s+>/g, '>')],
  ['play.html', read('../play.html').replace(/\s+/g, ' ').replace(/\s+>/g, '>')],
] as const;
...
describe('the Perfecting rail tile and keybind (the seven-piece exemplar)', () => {
  it('ships the tile in col-a of BOTH game entries, under Crafting, with its title and aria keys', () => {
    for (const [name, html] of entries) {
      ...
      expect(col, name).toMatch(/id="mm-perfecting"[^>]*>\s*<span class="keybind">s-t<\/span>/);
    }
  ...
  it('is dispatched at BOTH main.ts sites (keyboard onUiKey and dispatchGamepadAction)', () => {
    ...
    const gamepadStart = gamepadActionDispatchSrc.indexOf(
      'export function dispatchGamepadAction(id: string, deps: GamepadActionDeps): void {',
    );
    const gamepadRoute = gamepadActionDispatchSrc.slice(
      gamepadStart,
      gamepadActionDispatchSrc.indexOf('\n}', gamepadStart) + 2,
    );
...
describe('the Harvest Journal rail tile (the tile half over the existing Shift+K keybind)', () => {
  it('ships the tile in col-a of BOTH game entries, beside Professions', () => {
    for (const [name, html] of entries) {
      ...
      expect(col, name).toMatch(
        /id="mm-harvest-journal"[^>]*>\s*<span class="keybind">s-k<\/span>/,
      );
    }
