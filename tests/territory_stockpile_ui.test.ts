...
describe.each(['index.html', 'play.html'] as const)('%s territory stockpile markup', (path) => {
  it('shows every resource at once instead of using a selector', () => {
    ...
    expect(view.querySelectorAll('.territory-stockpile-resource-card')).toHaveLength(4);
    ...
    for (const resource of RESOURCE_KINDS) {
      ...
      expect(card?.querySelector(`img[src="/ui/items/territory_${resource}.webp"]`)).not.toBeNull();
      expect(card?.querySelector(`#territory-stockpile-${resource}-stored`)).not.toBeNull();
      expect(card?.querySelector(`#territory-stockpile-${resource}-bags`)).toBeNull();
      expect(card?.querySelector(`#territory-stockpile-${resource}-total`)).toBeNull();
      expect(card?.querySelector(`#territory-stockpile-${resource}-rate`)).toBeNull();
      expect(
        card?.querySelector(`[data-territory-stockpile-withdraw="${resource}"]`),
      ).not.toBeNull();
    }
  });
  ...
  it('puts hourly production inside each corresponding resource-building button', () => {
    ...
    for (const slot of resourceSlots) {
      ...
      expect(button?.querySelector(`#territory-slot-${slot}-production`)).not.toBeNull();
    }
  });
});
