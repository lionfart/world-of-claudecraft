...
### Required maintenance window for the material-source schema

The v0.42.1 update installs database triggers that reject legacy character-material
writes. Do not deploy this release as a rolling or mixed-version update: an older realm
writer can be rejected as soon as the first new process installs the triggers.

Before starting the update steps below, stop and fully drain every game/realm writer,
confirm all character saves have flushed, and take a restorable Postgres backup. Start
only v0.42.1-compatible processes after the migration completes. A binary-only rollback
to an older server is unsafe while the new triggers remain installed; either apply the
matching schema rollback or restore the pre-deploy database backup before bringing an
older writer back online.
