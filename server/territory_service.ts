...
import {
  ...
  type TerritorySiegeCatapultImpact,
  type TerritorySiegeControl,
  type TerritorySiegeMortarImpact,
  type TerritorySiegeRamImpact,
...
export type TerritoryCommand =
  ...
  | {
      ...
      /** Server-normalized siege-local mortar target. */
      aim?: { x: number; z: number };
      /** Server-owned actor facing at deployment time. */
      facing?: number;
    };
...
export interface TerritorySiegeInventoryAuthority {
  count(characterId: number, itemId: string): number;
  consume(characterId: number, itemId: string, count?: number): boolean;
  canGrant?(characterId: number, itemId: string, count: number): boolean;
  copper?(characterId: number): number;
  spendCopper?(characterId: number, amount: number): boolean;
  grant?(characterId: number, itemId: string, count?: number): void;
}

const EMPTY_SIEGE_INVENTORY: TerritorySiegeInventoryAuthority = {
  count: () => 0,
  consume: () => false,
  canGrant: () => false,
  copper: () => 0,
...
export class TerritoryService {
  ...
  > = [];
  private readonly pendingMortarImpacts: Array<
    TerritorySiegeMortarImpact & { warId: string; sourceCharacterId: number }
  > = [];
  private readonly pendingCatapultImpacts: Array<
  ...
  activeSiegeSlots(): ReadonlySet<number> {
    return new Set(this.siegeSlots.values());
  }

  /** Lets another battlefield mode reserve slots without coupling it to siege state. */
  setExternalOccupiedSlots(provider: () => ReadonlySet<number>): void {
    this.externalOccupiedSlots = provider;
  }

  drainTowerShots(): Array<{
    warId: string;
    characterId: number;
    damage: number;
    towerId: 'left' | 'right';
    ...
    this.siegePollFlight = (async () => {
      ...
      if (force || activation || hydrationDue) {
        ...
        const usedSlots = new Set([
          ...
          ...this.externalOccupiedSlots().values(),
        ]);
  ...
  tickSieges(
    ...
    for (const [warId, state] of this.sieges) {
      ...
      territorySiegeTick(state, nowMs, this.siegeRules);
      for (const launched of territorySiegeDrainLaunchedMortarImpacts(state)) {
        this.pendingMortarImpacts.push({
          ...launched.impact,
          warId,
          sourceCharacterId: launched.sourceCharacterId,
        });
        changed = true;
      }
  ...
  async execute(
    ...
    if (command.kind === 'siege_action') {
      ...
      for (const state of this.sieges.values()) {
        ...
        if (action.catapultImpact) {
          this.pendingCatapultImpacts.push({
            ...action.catapultImpact,
            warId: state.definition.warId,
            sourceCharacterId: characterId,
          });
        }
    ...
    if (command.kind === 'harvest') {
      ...
      if (
        ...
        !this.siegeInventory.canGrant?.(
          characterId,
          TERRITORY_RESOURCE_ITEM_IDS[command.resource],
          available,
        )
      ) {
      ...
      case 'harvest':
        ...
        if (result.ok && !result.duplicate && result.harvest) {
          this.siegeInventory.grant?.(
            characterId,
            TERRITORY_RESOURCE_ITEM_IDS[result.harvest.kind],
            result.harvest.amount,
          );
        }
        break;
      case 'build':
