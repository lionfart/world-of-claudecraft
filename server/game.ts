...
import { mobZonePhase, SIM_MOB_ZONE_PHASES } from './mob_zone_phase';
...
import { TerritoryRepository, territoryGame } from './territory_game_runtime';
import { formatMobZoneLine, formatSimPhaseLine, formatTickPerfLine, round2 } from './tick_perf_log';
...
export class GameServer {
  ...
  private readonly guildCreationFeeCopper: number;
  constructor(
    generalChatQuotaMaxInFlight = GENERAL_CHAT_QUOTA_MAX_IN_FLIGHT,
    backgroundDbGateOrGuildFee?: BackgroundDbGate | number,
  ) {
    this.backgroundDbGate =
      typeof backgroundDbGateOrGuildFee === 'number' ? undefined : backgroundDbGateOrGuildFee;
    this.guildCreationFeeCopper =
      typeof backgroundDbGateOrGuildFee === 'number'
        ? backgroundDbGateOrGuildFee
        : guildCreationFeeFromEnv();
    ...
    this.paidGuildCreation = createPaidGuildCreationCoordinator({
      ...
      chargeFee: (pid) => this.sim.chargeGuildCreationFeeFor(pid, this.guildCreationFeeCopper),
      feeCopper: this.guildCreationFeeCopper,
    ...
    this.guildBankLazyLoader = createGuildBankLazyLoader({
      ...
      ...(this.backgroundDbGate
        ? { tryAcquireImmediatePermit: () => this.backgroundDbGate?.tryAcquire() ?? null }
        : {}),
    ...
    this.interval = setInterval(() => {
      ...
      runGuarded(
        () => {
          ...
          feedRealmCalendar(this.sim, calendarNowMs, REALM_RESET_TIME_ZONE);
          this.bcastGridNs = 0n;
          this.bcastSelfNs = 0n;
