...
export function territorySiegeObjectiveSelectable(
  ...
  if (target.kind === 'wall')
    return (
      siege.wallHealth?.some(
        (entry) => entry.id === target.id && entry.hp > 0,
      ) ?? false
    );
  if (target.kind === 'tower')
    return (
      siege.defenseTowerLevel > 0 &&
      (siege.towerHealth?.some(
        (entry) => entry.id === target.id && entry.hp > 0,
      ) ??
        false)
    );
  ...
  if (target.kind === 'mortar')
    return siege.mortars.some((entry) => entry.id === target.id);
...
export function territorySiegeGuideVisibility(
  ...
  const towerVisible = (id: 'left' | 'right'): boolean => {
    if (
      !towersActive ||
      selectedObjective?.kind !== 'tower' ||
      selectedObjective.id !== id
    )
      return false;
...
export function territorySiegeVisualState(
  ...
  return {
    ...
    ramThrust:
      siege?.ramDeployed && siege.ramCooldown > 0
        ? Math.sin(timeSeconds * 8) * 0.72
        : 0,
