...
export type TerritoryStructureState = 'building' | 'active' | 'damaged' | 'repairing';
...
export type TerritorySiegeWallRun =
  | 'left'
  | 'right'
  | 'back'
  | 'front_left'
...
export interface TerritoryCityStockpileView {
  cellId: number;
  resources: Record<TerritoryResourceKind, number>;
  productionPerHour: Record<TerritoryResourceKind, number>;
  /** Per-resource storage ceiling supplied by this city's Stockpile. */
  capacity: number;
}
...
export interface TerritorySiegeView {
  ...
  /** One authoritative tier shared by the keep core, gate, walls, and courtyard. */
  castleLevel: number;
  /** Durable target-city buildings rendered at their real transferred level. */
...
export interface TerritoryGuildView {
  ...
  /** Uncollected production still held by the guild's territory ledger. */
  resources: Record<TerritoryResourceKind, number>;
  /** Viewer-private spendable stacks currently carried in bags. */
  inventoryResources?: Record<TerritoryResourceKind, number>;
  /** Server-authoritative production generated by all owned cities in one hour. */
  productionPerHour?: Record<TerritoryResourceKind, number>;
  /** Private per-city balances. Stored goods belong to the land, not the guild. */
  stockpiles?: TerritoryCityStockpileView[];
  resourceCapacity: number;
  accruedAt: string;
}

export interface TerritoryCaptureView {
  id: string;
  cellId: number;
  slot: number;
  biome: TerritorySiegeBiome;
  ...
  occupants: number;
  progress: number;
  /** Viewer-private registration state; starting a camp does not register its officer. */
  registered: boolean;
  /** Guild-visible roster for the active neutral-camp battle. */
  members: TerritoryCaptureMemberView[];
  /** This character is physically inside the expedition battlefield. */
  inField: boolean;
  /** Seconds before this registered character is transferred into the field. */
  preparingIn: number;
  respawnIn: number;
  /** Registration and re-entry remain valid for this many seconds. */
  timeLeft: number;
  secondsRemaining: number;
}

export interface TerritoryCaptureMemberView {
  characterId: number;
  name: string;
  connected: boolean;
  inField: boolean;
...
export interface TerritoryMapState {
  ...
  /** Present only for a character currently contesting an unclaimed camp. */
...
export interface IWorldTerritory {
  ...
  territoryHarvest(cellId: number, resource: TerritoryResourceKind): void;
  territoryCaptureAction(action: 'join' | 'enter' | 'leave'): void;
  ...
  /** Repairs a captured non-castle structure without changing its level. */
