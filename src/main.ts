...
async function startGame(
  ...
  const groundAimReticleSync = createGroundAimReticleSync({
    ...
    cursorPoint: () => combatAim.screenPoint(),
  ...
  const sim = loadSpan(
    ...
    () =>
      new Sim({
        ...
        // Directional combat is the local fork's normal runtime contract. Keep
        // the Sim library opt-in so deterministic fixtures remain unchanged.
