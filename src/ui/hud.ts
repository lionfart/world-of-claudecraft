...
import { ChatAnnouncer } from './hud/chat';
...
export class Hud {
  ...
  private readonly groundAim = new GroundAimController({
    ...
    castAt: (id, point) => {
      const resolved = this.sim.known.find((known) => known.def.id === id) ?? null;
      if (resolved?.def.targetMode === 'position' && !resolved.def.selfCentered) {
        this.sim.castAbilityAt(id, point);
      } else if (resolved) {
        this.castResolvedAbility(id, resolved);
      } else {
  ...
  private mapLevel: MapLevel | 'territory' = 'zone';
  ...
  private drawTerritoryObjectivePortrait(kind: TerritorySiegeObjectiveTarget['kind']): void {
    ...
    shade.addColorStop(0, TERRITORY_PORTRAIT_TONE.SHADE_INNER);
    shade.addColorStop(1, TERRITORY_PORTRAIT_TONE.SHADE_OUTER);
    context.fillStyle = shade;
    context.beginPath();
    context.arc(29, 29, 27, 0, Math.PI * 2);
    context.fill();
    context.strokeStyle = TERRITORY_PORTRAIT_TONE.BORDER;
    context.lineWidth = 2;
    context.stroke();
    if (kind === 'gate' || kind === 'wall' || kind === 'tower') {
      context.fillStyle = TERRITORY_PORTRAIT_TONE.STONE;
      context.fillRect(10, 14, 8, 33);
      context.fillRect(40, 14, 8, 33);
      context.fillRect(10, 10, 38, 8);
      context.fillStyle =
        kind === 'gate' ? TERRITORY_PORTRAIT_TONE.GATE : TERRITORY_PORTRAIT_TONE.WALL;
      context.fillRect(18, 18, 22, 29);
      context.strokeStyle = TERRITORY_PORTRAIT_TONE.STONE_OUTLINE;
      context.lineWidth = 2;
    ...
    context.fillStyle = TERRITORY_PORTRAIT_TONE.CART;
    context.fillRect(13, 25, 32, 13);
    context.strokeStyle = TERRITORY_PORTRAIT_TONE.WHEEL;
    context.lineWidth = 3;
    context.beginPath();
  ...
  private groundReticleEnabled(_abilityId?: string): boolean {
  ...
  cancelGroundAim(): boolean {
    ...
    const groundReticleCleared = this.groundAim.cancel();
    this.territoryMortarAimSlot = null;
    this.territoryCatapultAimSlot = null;
    this.groundAimPoint = null;
    this.groundAimClamped = false;
    if (!groundReticleCleared) this.renderer.setGroundAimReticle(null);
    this.renderer.setAbilityRangeReticle(null);
  ...
  private paintPreparedSlot(slot: number | null): void {
    const abilityId = slot !== null ? this.groundAim.activeAbilityId() : null;
    const prepared = abilityId
      ? (this.sim.known.find((known) => known.def.id === abilityId) ?? null)
      : null;
  ...
  groundAimAbilityRange(): number | null {
    if (this.territoryCatapultAimSlot !== null) return TERRITORY_SIEGE_CATAPULT_RANGE;
    if (this.territoryMortarAimSlot !== null) return TERRITORY_SIEGE_MORTAR_RANGE;
  ...
  nudgeGroundAimPoint(dx: number, dz: number): void {
    if (this.territoryMortarAimSlot !== null || this.territoryCatapultAimSlot !== null) {
      const point = this.groundAimPoint ?? this.groundTargetAim();
      this.updateGroundAimPoint({ x: point.x + dx, z: point.z + dz });
      return;
    }
  ...
  private castResolvedAbility(abilityId: string, resolved: ResolvedAbility): void {
    ...
    const mouseoverPid = mouseoverCastTargetPid(this.hoveredPartyPid, def, {
    ...
    if (mouseoverPid !== null) {
      this.sim.castAbilityOn(abilityId, mouseoverPid);
      return;
    }
    this.castActionAbility(abilityId, def);

    const targetId = this.sim.player.targetId;
    const target = targetId !== null ? (this.sim.entities.get(targetId) ?? null) : null;
    if (
      this.optionsHooks?.settings.get('startAttackOnAbilityUse') &&
      abilityStartsAutoAttack(resolved.effects) &&
      hasAutoAttackTarget(
        target,
        isPvpHostileTarget(targetId, this.sim.duelInfo, this.sim.arenaInfo, this.sim.bgInfo),
      )
    ) {
      if (deferAutoAttackUntilCastEnd(resolved.castTime)) {
        this.pendingAutoAttackAbilityId = abilityId;
      } else {
        this.sim.startAutoAttack();
      }
    }
  ...
  commitGroundAimAt(rawPoint?: AimPoint | null): boolean {
    ...
    if (this.territoryCatapultAimSlot !== null) {
      ...
      const raw = rawPoint ?? this.groundAimPoint ?? this.groundTargetAim();
    ...
    const committed = this.groundAim.commitAt(rawPoint);
    if (committed) this.paintPreparedSlot(null);
    return committed;
  }
  ...
  castSlot(barSlot: number): void {
    ...
    if (action?.type === 'ability') {
      ...
      if (resolved) {
        ...
        if (
          shouldPrepareAbility(resolved.def, mobileTouch, this.groundReticleEnabled(action.id)) &&
  ...
  update(paint = true): void {
    ...
    this.toggleClass(
      this.targetFrameEl,
      'territory-objective',
      !(target && target.kind !== 'object') && territoryObjective !== null,
    );
    if (target && target.kind !== 'object') {
      ...
      this.paintTerritorySiegeObjective(territoryObjective);
    } else {
  ...
  private toggleMapLevel(): void {
    ...
    if (mode === 'overworld' && this.mapLevel === 'continent') {
      this.setMapLevel('territory');
      return;
    }
    this.setMapLevel(nextMapLevel(mode, this.mapLevel, remoteInstanceAnchor(this.sim) !== null));
  }

  private setMapLevel(level: MapLevel | 'territory'): void {
    ...
    this.setText(
      ...
      t(
        this.mapLevel === 'territory'
          ...
          : mapLevelToggleKey(mapMode, this.mapLevel, !!remote),
      ),
    );
      ...
      switch (ev.type) {
        ...
        case 'castStop':
          if (ev.entityId === sim.playerId && this.pendingAutoAttackAbilityId !== null) {
            ...
            if (ev.success) {
              const targetId = sim.player.targetId;
              const target = targetId !== null ? (sim.entities.get(targetId) ?? null) : null;
              if (
                hasAutoAttackTarget(
                  target,
                  isPvpHostileTarget(targetId, sim.duelInfo, sim.arenaInfo, sim.bgInfo),
                )
              ) {
                sim.startAutoAttack();
              }
  ...
  private logScrollFollower(el: HTMLElement): LogScrollFollower {
    const follower = el === this.chatLogEl ? this.chatLogScroll : this.combatLogScroll;
    return follower ?? new LogScrollFollower();
  }
