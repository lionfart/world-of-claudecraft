...
function applyAbility(
  ...
  if (ability.impactArea && ability.targetMode === 'position') {
    ...
    if (primary) {
      ...
      for (let index = 0; index < targets.length; index++) {
        const areaTarget = targets[index];
        const resisted =
          ability.school !== 'physical' &&
          ctx.isHostileTo(p, areaTarget) &&
          !res.effects.some((effect) => effect.type === 'taunt') &&
          resolveHostileSpellResist(ctx, p, areaTarget, ability);
        if (resisted) {
          if (index === 0) primaryResisted = true;
          continue;
        }
        if (index === 0) {
          ctx.runEffects(p, meta, areaTarget, res);
          continue;
        }
        if (ctx.runSecondaryTargetEffects) {
          ctx.runSecondaryTargetEffects(p, meta, areaTarget, res, secondarySnapshot);
        } else {
          ctx.runEffects(p, meta, areaTarget, res);
        }
      }
      if (primaryResisted) restoreStormcastReservation(ctx, p, stormcastReservation);
      else completeStormcastReservation(ctx, p, stormcastReservation);
    } else {
  ...
  if (multiTargetMelee) {
    ...
    let primaryResisted = false;
    for (let i = 0; i < directionalTargets.length; i++) {
      const meleeTarget = directionalTargets[i];
      const resisted =
        ability.school !== 'physical' &&
        ctx.isHostileTo(p, meleeTarget) &&
        !res.effects.some((eff) => eff.type === 'taunt') &&
        resolveHostileSpellResist(ctx, p, meleeTarget, ability);
      if (resisted) {
        if (i === 0) primaryResisted = true;
        continue;
      }
      if (i === 0) {
    ...
    // rolls on impact; only the delivery differs. Taunts remain exempt.
