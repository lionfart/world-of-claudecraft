...
export class Sim {
  ...
  constructor(cfg: SimConfig) {
    ...
    if (!cfg.noPlayer) {
      this.ownPlayerPid = this.addPlayer(this.cfg.playerClass, this.cfg.playerName, {
        ...
        // Allocated by the host outside deterministic simulation.
  ...
  /** Applies the player-only secondary payload of an authoritative mortar hit. */
  ...
  startTutorial(pid?: number): void {
    ...
    if (!resolved) return;
    resolveStartTutorial(this.ctx, resolved.e, resolved.meta);
  }
