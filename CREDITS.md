...
| HUD chrome launcher icons (`public/ui/chrome/*.webp`, one per primary destination in the side rail and mobile bar, keyed to alpha and normalized to 128px WebP via `scripts/convert_chrome_icons_webp.mjs`) | World of ClaudeCraft | Project-generated with Google Gemini and OpenAI built-in image generation to one authored art direction (the Reliquary crown is authored in-repo as layered SVG to the same direction, source under `scripts/assets/chrome_crown/`), keyed and optimized locally; exact provenance per icon in `public/ui/chrome/mapping.json` | Project asset, rights reserved | **No, permission required** |
| Territory field-mortar inventory painting (`public/ui/items/territory_field_mortar.webp`) | World of ClaudeCraft | Original compact medieval field-mortar icon generated with OpenAI built-in image generation, normalized locally to a 128px WebP; exact prompt and accepted source are retained in `public/ui/items/mapping.json` batch `territory-field-mortar-2026-09-02` | Project asset, rights reserved | **No, permission required** |
| Territory field-catapult model and inventory painting (`public/models/siege/territory_catapult.glb`, `public/ui/items/territory_catapult.webp`) | World of ClaudeCraft project owner | Owner-provided `Catapult.obj` and texture set, normalized, material-repaired, web-optimized, and rendered locally in Blender; reproducible scripts and accepted icon source are retained under `scripts/assets/territory_catapult/`, with item lineage in `public/ui/items/mapping.json` batch `territory-field-catapult-2026-09-03` | Used with permission (Levy Street) | **No, permission required** |
| Territory siege-weapon ability icons (`public/ui/skills/territory/*.webp`) | World of ClaudeCraft | Original seven-icon ram, mortar, and catapult action-bar atlas generated with OpenAI built-in image generation and deterministically cropped/normalized locally; exact prompt, accepted source, and ability mapping are retained in `public/ui/skills/territory/mapping.json` | Project asset, rights reserved | **No, permission required** |
...
| Ignivar raid ambient suite (`ignivar_forge_approach`, `ignivar_raid_arena`, `ignivar_inner_crucible`) | World of ClaudeCraft | Original project-authored procedural score, rendered with `scripts/render_music.mjs` and remastered locally | Project asset | With the project only |
